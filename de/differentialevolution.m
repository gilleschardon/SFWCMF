...
while ...
    ...
    for memberNr = 1:NP
      ...
      if initialization && (memberNr == 1)
        ...
        displaybestmember(paramDefCell, parameterDimVector, initval, initmem, ...
                          allval, initval, initmem, 1, state, optimInfo, sendEmail);
      ...
      displaybestmember(paramDefCell, parameterDimVector, bestval, bestmem, allval, ...
                        initval, initmem, 0, state, optimInfo, sendEmail);
  ...
  for memberNr = 1:NP
    ...
    displaybestmember(paramDefCell, parameterDimVector, bestval, bestmem, ...
                      allval, initval, initmem, 0, state, optimInfo, sendEmail);
...
displaybestmember(paramDefCell, parameterDimVector, bestval, bestmem, allval, ...
                  initval, initmem, 1, state, optimInfo, sendEmail);
...
for k = 1:maxNrOfTests
  ...
  if any(all(abs(quantPop(1:n-1, :) - quantPop(n(ones(n-1, 1)), :)) < eps, 2)) || ...
    ...
    % quantized member unique and valid, step to next member
    nindex = nindex + 1;
  end
end
...
if forceSaveFile || mbtime - nextSaveTime >= 0
  ...
  if ~isempty(bestmem)
    ...
    % check which boundary values are reached
    for parNr = 1:D
...
function displaybestmember(...
  ...
  forceParameterDisplay, state, optimInfo, sendEmail)
