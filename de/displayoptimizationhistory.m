...
if any(parGridIndex)
	allmemImg(parGridIndex,:) = round(...
    (allmem(parGridIndex,:) - repmat(parameterBounds(parGridIndex,1), 1, size(allmem,2))) ./ ...
    repmat(parGridVector(parGridIndex), 1, size(allmem,2)))+1;
end
...
for k=1:nOfFigures
	...
	selectfigure(sprintf('Parameters %d and %d', 2*k-1, 2*k), 'Position', defaultFigurePosition); clf;
	...
	createImage = all(parGridVector(2*k-1:2*k) > 0) && ...
		all(diff(parameterBounds(2*k-1:2*k,1:2), 1, 2) ./ parGridVector(2*k-1:2*k) < 500);
	if createImage
