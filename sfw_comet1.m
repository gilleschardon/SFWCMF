function [Xs, amps, v, nu, Pest] = sfw_comet1(Xm, k, Data, Xgrid, tolpos, tolamp, Niter, LX, UX)
...
options_amps = optimoptions(@fmincon,'Display', 'off', 'Algorithm','sqp', 'SpecifyObjectiveGradient',true, 'CheckGradient', false, 'OptimalityTolerance', 1e-12);
...
function [J, Jgrad] = obj_amplitudes(D, Data, Datainv, x, lbv)
...
Jgrad = zeros(size(x));
if nargout > 1
    Jgrad(1:end-1) = ones(size(x(1:end-1))) * 0;
    Jgrad(1:end-1) = Jgrad(1:end-1) + real(sum( (Datainv*D) .* conj(D), 1))'; 
    Jgrad(1:end-1) = Jgrad(1:end-1) - real(sum( ((Cinv*Data*Cinv)*D) .* conj(D), 1))'; 

    Jgrad(end) = real(trace(Datainv)  - trace(Cinv*Data*Cinv));
end
